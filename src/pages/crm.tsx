...
export function Crm() {
  ...
  // Exporta para CSV exatamente o que está na tela no momento do clique:
  // respeita os filtros ativos (linhas) e as colunas ocultas/visíveis, na
  // mesma ordem em que aparecem na planilha.
  const exportarCsv = useCallback(() => {
    const colunasVisiveis = COLUNAS_CRM.filter((c) => !oculta(c.key));
    const campoDoContato = (c: Contato, key: ColunaCrm): string => {
      switch (key) {
        case "pessoal":
          return c.pessoal ? "Sim" : "Não";
        case "atendido":
          return c.atendido ? "Sim" : "Não";
        case "nome":
          return c.nome || "";
        case "empresa":
          return c.empresa || "";
        ...
        case "email":
          return c.email || "";
        case "relacionamento":
          return String(c.relacionamento ?? "");
        default:
          return "";
      }
    };
    const escaparCsv = (v: string) => `"${v.replace(/"/g, '""')}"`;
    const cabecalho = colunasVisiveis.map((c) => escaparCsv(c.label)).join(",");
    const linhas = filtrados.map((c) =>
      colunasVisiveis.map((col) => escaparCsv(campoDoContato(c, col.key))).join(","),
    );
    ...
    a.href = url;
    a.download = `crm-contatos-${carimbo}.csv`;
    a.click();
    setTimeout(() => URL.revokeObjectURL(url), 60_000);
  }, [filtrados, colunasOcultas]);
